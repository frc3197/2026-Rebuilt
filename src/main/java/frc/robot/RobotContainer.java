...
public class RobotContainer {
    ...
    private final ControlScheme controlScheme = new ControlScheme(driveController, secondaryController);

    // Dashboard inputs
    private final LoggedDashboardChooser<Command> autoChooser;

    private final NetworkButton seedRightAutoRed = new NetworkButton(NetworkTableInstance.getDefault(),
            "SmartDashboard", "Seed right red");

    // Triggers
    // -------------------------------------------------------------------------
    // Trigger for when robot enters the alliance zone, used to automatically begin
    // tracking hub
    private final Trigger enteredAllianceZoneAuto = new Trigger(
    ...
    private final Trigger enteredNeurtalZoneTele = new Trigger(
            () -> (RobotState.instance().inNeutralZone()
                    && (LoggingConstants.currentMode == Mode.REAL && !DriverStation.isAutonomous())
                    && !DriverStation.isTest()));

    private final Trigger enteredAllianceZoneTele = new Trigger(
            () -> (RobotState.instance().inAllianceZone()
                    && (LoggingConstants.currentMode == Mode.REAL && !DriverStation.isAutonomous())
                    && !DriverStation.isTest()));

    private final Trigger aboutToBeActive = new Trigger(MatchTimeUtil.instance().aboutToBecomeActiveSupplier());

    // Start flopping the intake when the shooter is spooling and the robot is below
    // a certain speed
    private final Trigger startFlopping = new Trigger(
            () -> (!DriverStation.isAutonomous()
                    && RobotState.instance().getClimberAngle().lt(Degrees.of(10))
                    && RobotState.instance().getRobotSpeedMPS() < IntakeConstants.MAX_FLOP_VELOCITY.in(MetersPerSecond)
                    && (RobotState.instance().getFlywheelMode() == FlywheelMode.FRENZY
                            || RobotState.instance().getFlywheelMode() == FlywheelMode.SHOOTING)));

    private final Trigger hoodDownTrigger = new Trigger(() -> {
        return RobotState.instance().robotNearBottomBlueTrench() || RobotState.instance().robotNearTopBlueTrench()
                || RobotState.instance().robotNearBottomRedTrench() || RobotState.instance().robotNearTopRedTrench();
    });
                        ...
                        Commands.runOnce(
                                ...
                                .andThen(
                                        Commands.run(
                                                ...
                                                .andThen(
                                                        Commands.runOnce(
                                                                () -> index.setFeedRequest(new VoltageOut(0.0)),
                                                                index))));
        ...
        aboutToBeActive
                .onTrue(
                        Commands.runOnce(
                                ...
                                .andThen(
                                        Commands.run(() -> driveController.setRumble(RumbleType.kBothRumble, 0.5))))
                .onFalse(Commands.run(() -> driveController.setRumble(RumbleType.kBothRumble, 0.0)));
